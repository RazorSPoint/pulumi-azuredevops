PROJECT_NAME := Microsoft Azure DevOps Package
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-azuredevops,${REPOSITORY},g' go.mod; \
...
build:: patch tfgen provider
	...
	for LANGUAGE in "python" "go" "dotnet" ; do \
...
tfgen::
	go install -mod=${TF_MOD} -ldflags "-X github.com/pulumi/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}

provider::
	go install -mod=${TF_MOD} -ldflags "-X github.com/pulumi/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}

lint::
	golangci-lint run

install::
	GOBIN=$(PULUMI_BIN) go install -mod=${TF_MOD} -ldflags "-X github.com/pulumi/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
